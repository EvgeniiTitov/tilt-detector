...
from tilt_detector.abstract_angle_calculator import AbstractAngleCalculator
from tilt_detector.utils import calculate_angle
from tilt_detector.utils import draw_lines_write_text


class PoleAngleCalculator(AbstractAngleCalculator, LoggerMixin):
    def __init__(
        self,
        draw_detected_edges: bool,
        angle_thresh: int = 70,
        min_merge_distance: int = 30,
        min_merge_angle: int = 30,
    ) -> None:
        self._draw_edges = draw_detected_edges
        self._angle_thresh = angle_thresh
        self._min_distance_to_merge = min_merge_distance
        self._min_angle_to_merge = min_merge_angle
        self.logger.info("PoleAngleCalculator initialized")

    def calculate_inclination(
        self, image: np.array
    ) -> t.Tuple[t.Optional[float], t.Optional[list]]:
        the_edges = self.find_pole_edges(image)
        if the_edges:
            the_angle = calculate_angle(the_edges)
            if self._draw_edges:
                draw_lines_write_text(the_edges, image, the_angle)
            return the_angle, the_edges
        else:
            return None, None
